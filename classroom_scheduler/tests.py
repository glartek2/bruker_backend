...
class ReservationUpdateTest(APITestCase):
    def setUp(self):
        self.instructor = CustomUser.objects.create_user(username="instructor",  email='test@example.com',password="test123")
        self.b1 = Building.objects.create(
                        name="Test Hala", address="ul. Testowa 1",
                        department="D", description="Desc"
                    )
        ...
        self.new_room = Room.objects.create(
            building=self.b1, equipment=self.e1,
            room_number="R2", capacity=25
        )

        self.group = ClassGroup.objects.create(name="Group A")
        self.group.instructors.add(self.instructor)

        self.info = ReservationInfo.objects.create(user=self.instructor, group=self.group, description="desc")
        self.reservation = Reservation.objects.create(
            room=self.room,
            date_time=make_aware(datetime(2025, 6, 20, 10, 0)),
            reservation_info=self.info
        )
        self.client.force_authenticate(user=self.instructor)


    def test_instructor_can_update_reservation_directly(self):
        url = f'/api/reservation/{self.reservation.id}/'
        payload = {
            "proposed_room_id": self.new_room.id,
            "proposed_date_time": "2025-06-25T10:00:00Z"
        }
        ...
        self.assertEqual(self.reservation.room, self.new_room)
        self.assertEqual(self.reservation.date_time, make_aware(datetime(2025, 6, 25, 10, 0)))
        self.assertIsNone(self.reservation.proposed_room)
        self.assertIsNone(self.reservation.proposed_date_time)
